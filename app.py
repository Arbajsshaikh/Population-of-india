...
def load_data(state, district, taluka):
    file_path = os.path.join("population", state, district )
    df = pd.read_excel(file_path, sheet_name=taluka)
    return df

# Main function to run the Streamlit app
def main():
    st.title("Population Data Viewer")

    # Get list of states
    states = os.listdir("population")

    # Dropdown to select state
    selected_state = st.selectbox("Select State", states)

    # Get list of districts in selected state
    districts = os.listdir(os.path.join("population", selected_state))

    # Dropdown to select district
    selected_district = st.selectbox("Select District", districts)

    # Get list of talukas in selected district
    talukas = pd.ExcelFile(os.path.join("population", selected_state, selected_district )).sheet_names
